...
class NewResourceCommand(sublime_plugin.WindowCommand):

    def run(self):
        self.window.show_input_panel("New resource name:", "", self.on_done, None, None)
        pass

    def on_done(self, text):
        if self.window.active_view():
            self.window.active_view().run_command("ibld_exec", {"text": text})
...
class IbldExecCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, text):
        ...
        sublime.status_message("Creating new resource")
