...
class BaseCommand(sublime_plugin.WindowCommand):
    ...
    def setup_data_from_settings(self):
        self.settings = sublime.load_settings("bis-sublime.sublime-settings")
    ...
    def show_output_panel(self, text):
        ...
        if self.results_in_new_tab:
            new_tab_path = os.path.join(self.bis_sublime_results_path(), "Gulp Results")
            self.output_view = self.window.open_file(new_tab_path)
            self.output_view.set_scratch(True)
        else:
            self.output_view = self.window.get_output_panel("bis_sublime_output")
            self.show_panel()
    ...
    def bis_sublime_results_path(self):
    ...
    def close_panel(self):
        if self.results_in_new_tab:
            ...
            self.window.run_command("hide_panel", {"panel": "output.bis_sublime_output"})

    def show_panel(self):
        self.window.run_command("show_panel", {"panel": "output.bis_sublime_output"})
    ...
    def async(self, fn, delay):
        if is_sublime_text_3:
            progress = ProgressNotifier('BIS Sublime: Working')
